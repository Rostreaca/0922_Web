	...
	<div class="outer">
		...
		<div class="row">
		  <div class="offset-lg-2 col-lg-8">
		    <div class="card">
		      <div class="card-header text-white" style="background-color: #52b1ff;">${ map.notice.boardNo }번 게시물 내용</div>
		      <div class="card-body"> 
		        
		          <div class="form-group">
		            <label>작성자</label>
		            <input type="text" class="form-control" name='writer' value="${ map.notice.boardWriter }" readonly>
		          </div>
		          
		          <div class="form-group">
		            <label>제목</label>
		            <input type="text" class="form-control" name='title' value="${ map.notice.boardTitle }" readonly>
		          </div>
		
		          <div class="form-group">
		            <label>내용</label>
		            <textarea class="form-control" rows="5" name='content' readonly style="resize:none;">${ map.notice.boardContent }</textarea>
		          </div>
		          ...
		          <a class="btn" href="notice?page=1"
		          ...
		          <c:if test="${ userInfo.userNo eq map.boardWriter }">
			          <a 
			            class="btn" 
			            href="updateForm.no?boardNo=${ map.notice.boardNo }"
			      		style="background-color: orange; height: 40px; color: white; border: 0px solid #388E3C; opacity: 0.8"
			      		>수정</a>&nbsp;&nbsp;
			          
			          <a 
			            class="btn" 
			            href="delete.no?boardNo=${ map.notice.boardNo }" onclick="return confirm('정말로 삭제하시겠습니까?')"
